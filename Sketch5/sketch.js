...
function preload() {
  mySound = loadSound("assets/B5.mp3");
}
...
function updateCreature(index) {
    ...
    let f = map(mic.getLevel(), 0, 1, 1, 50);
    positionsX[index] += velocitiesX[index] * f;
    positionsY[index] += velocitiesY[index] * f;
    // Bounce off walls
...
function drawCreature(index) {
    ...
    // Wings
    let s = map(mic.getLevel(), 0,1, 0.1, 1);
    let wingAngle = sin(frameCount * s) * PI / 6;
